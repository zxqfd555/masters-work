...
const int32_t HYPERPARAMS_NUM = 6;
...
private:
    ...
    void PrecalculateFeatures () {
        ...
        const auto wordRanks = NTextRank::GetKeywords(TokenizedText, 1000);
        map<string, int32_t> strongWordRanks;
        for (auto&& word : wordRanks) {
            strongWordRanks[word.Value] = (int32_t)(word.Rank * 1000000);
        }
        ...
        for (int32_t ngramLength = 1; ngramLength <= 3; ++ngramLength) {
            for (int32_t firstWordIdx = 0; firstWordIdx < (int32_t)TokenizedText.size() - ngramLength + 1; ++firstWordIdx) {
                ...
                features[5] = 1e9;
                for (size_t i = 0; i < ngramLength; ++i) {
                    features[5] = min(features[5], strongWordRanks[TokenizedText[firstWordIdx + i]]);
                }
...
public:
    ...
    size_t TestOwnTextrank() {
        auto result = NTextRank::GetKeywords(TokenizedText);

        set<string> tagsSet;
        for (auto&& actualTag : ActualTags) {
            tagsSet.insert(actualTag);
        }

        size_t nHits = 0;
        for (auto&& word : result) {
            if (tagsSet.find(word.Value) != tagsSet.end()) {
                nHits += 1;
            }
        }
...
public:
    ...
    void ReadInputData(
        ...
        featuresTestFile.open("features-test.txt");
        for (size_t i = 1000; i < Texts.size(); ++i) {
            Texts[i].DumpFeatures(featuresTestFile);
        }
        featuresTestFile.close();
        
        ofstream scorerTestFile;
        scorerTestFile.open("test.txt");
        for (size_t i = 1000; i < Texts.size(); ++i) {
            Texts[i].DumpFeatures(scorerTestFile, i);
        }
        scorerTestFile.close();
        
        if (DEBUG) {
            size_t globalHits = 0;
            for (auto&& text : Texts) {
                globalHits += text.TestOwnTextrank();
            }
            cerr << "global hits of own textrank: " << globalHits << endl;
        }
    }
